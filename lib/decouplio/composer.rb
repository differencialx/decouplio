...
module Decouplio
  class Composer
    def self.compose(logic_container_raw_data:, palp_prefix: :root, parent_flow: {})
    ...
    def self.prepare_raw_data(flow, palp_prefix)
    ...
    def self.validate_flow(flow, palps)
      OptionsValidator.new(flow: flow, palps: palps).call
    end

    def self.extract_palp_flow(stp)
    ...
    def self.extract_palp_pool(stp)
      return {} unless stp[:type] == Decouplio::Const::Types::OCTO_TYPE
    ...
    def self.compose_flow(flow, palps, flow_hash = {})
      flow.each_with_index do |(step_id, stp), idx|
        ...
        when Decouplio::Const::Types::STEP_TYPE,
          ...
          stp[:flow][Decouplio::Const::Results::PASS] = next_success_step(flow.values, idx, flow[step_id][:on_success])
          stp[:flow][Decouplio::Const::Results::FAIL] = next_failure_step(flow.values, idx, flow[step_id][:on_failure])
        ...
        when Decouplio::Const::Types::IF_TYPE_FAIL
          ...
          stp[:flow][Decouplio::Const::Results::FAIL] ||= flow_hash[Decouplio::Const::Results::FAIL]
        when Decouplio::Const::Types::UNLESS_TYPE_PASS
          stp[:flow][Decouplio::Const::Types::PASS] = next_success_step(flow.values, idx, nil)
          stp[:flow][Decouplio::Const::Types::FAIL] = next_success_step(flow.values, idx + 1, nil)
          stp[:flow][Decouplio::Const::Types::FAIL] ||= flow_hash[Decouplio::Const::Results::PASS]
        when Decouplio::Const::Types::UNLESS_TYPE_FAIL
          stp[:flow][Decouplio::Const::Results::PASS] = next_failure_step(flow.values, idx, nil)
          stp[:flow][Decouplio::Const::Results::FAIL] = next_failure_step(flow.values, idx + 1, nil)
          stp[:flow][Decouplio::Const::Results::FAIL] ||= flow_hash[Decouplio::Const::Results::FAIL]
    ...
    def self.compose_action(stp)
      # TODO: && stp is_a_step_type, means that wrap, strategy, palp, resq does not accept action: key, add validations and tests
    ...
    def self.compose_strategy(stp, palps)
      return stp unless stp[:type] == Decouplio::Const::Types::OCTO_TYPE

      stp[:hash_case] = stp[:hash_case].map do |strategy_key, options|
        strategy_flow = compose(logic_container_raw_data: palps[options[:palp]], palp_prefix: options[:palp], parent_flow: stp[:flow])
        [
          strategy_key,
          strategy_flow
        ]
    ...
    def self.compose_resq(stp, palp_prefix)
      ...
      stp[:type] = Decouplio::Const::Types::STEP_TYPE_TO_RESQ_TYPE.dig(stp[:step_to_resq][:type])
    ...
    def self.random_id(name:, palp_prefix:)
